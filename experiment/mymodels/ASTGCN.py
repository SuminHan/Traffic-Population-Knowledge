...
class Spatial_Attention_layer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.W3 =  self.add_weight(shape=(self.in_channels, 1), initializer='glorot_uniform', dtype=tf.float32, name='W3')
    ...
    def call(self, x):
        ...
        # rhs = (self.W3 @ tf.transpose(x, (0, 1, 3, 2))
        rhs = tf.squeeze((tf.transpose(x, (0, 1, 3, 2)) @ self.W3), -1)
        ...
        # S = S - tf.reduce_max(S, 1, keepdims=True)
...
class cheb_conv_withSAt(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.Theta = [self.add_weight(shape=(self.in_channels, self.out_channels), initializer='glorot_uniform', name=f'K{i}') for i in range(self.K)]
    ...
    def call(self, x, spatial_attention):
        ...
        print('cheb_conv_withSAt', x.shape)
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                ...
                T_k_with_at = tf.multiply(T_k, spatial_attention)   # (N,N)*(N,N) = (N,N) 多行和为1, 按着列进行归一化
...
class Temporal_Attention_layer(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        # E = E - tf.reduce_max(E, 1, keepdims=True)
...
class cheb_conv(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # self.Theta = self.add_weight(shape=(self.K, self.in_channels, self.out_channels), initializer='glorot_uniform', name='Kc')
        self.Theta = [self.add_weight(shape=(self.in_channels, self.out_channels), initializer='glorot_uniform', name=f'K{i}') for i in range(self.K)]
    ...
    def call(self, x):
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                ...
                rhs = tf.transpose(tf.transpose(graph_signal, (0, 2, 1)) @ T_k, (0, 2, 1))
...
class ASTGCN_block(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # self.cheb_conv = cheb_conv(self.K, self.cheb_polynomials, self.in_channels, self.nb_chev_filter)
        ...
        self.ln = layers.LayerNormalization(axis=2)  #需要将channel放到最后一个维度上
    ...
    def call(self, x):
        ...
        x_residual = self.ln(tf.transpose((tf.transpose(tf.nn.relu(x_residual + time_conv_output), (0, 3, 2, 1))), (0, 2, 3, 1)))
...
class ASTGCN_submodule(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.final_conv = layers.Conv2D(1, kernel_size=(1, num_for_predict))
        
        # D = self.nb_time_filter
        # self.final_layer_DCGRU = tf.keras.layers.RNN(DCGRUCell(D, cheb_polynomials[0], 1, self.num_of_vertices, 'random_walk'), return_state=False)
        # self.final_layer_FC = keras.Sequential([
    ...
    def call(self, x):
        ...
        x = tf.transpose(x, (0, 1, 3, 2)) # (B, N, T, F)
        x = self.final_conv(x) # (B, N, 1, 1)
        x = tf.squeeze(x, -1)
        output = tf.transpose(x, (0, 2, 1))



        # x = tf.transpose(x, (0, 3, 1, 2)) # (B, T, N, F)
        # x = self.final_layer_DCGRU(x)
        # x = tf.reshape(x, (-1, self.num_of_vertices, self.nb_time_filter))
        # output = self.final_layer_FC(x)
        # output = tf.transpose(output, (0, 2, 1)) 
...
class MyASTGCNSTE(tf.keras.layers.Layer):
    def __init__(self, extdata, args):
        super(MyASTGCNSTE, self).__init__()
    ...
    def build(self, input_shape):
        ...
        self.FC_XC_in = keras.Sequential([
                            layers.Dense(D, activation="relu"),
                            layers.Dense(D)])
        # self.FC_XC_DCGRU = tf.keras.layers.RNN(DCGRUCell(D, self.adj_mat, 1, self.num_nodes, 'random_walk'), return_state=False, return_sequences=True)
