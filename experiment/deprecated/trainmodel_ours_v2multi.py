...
(trainX, trainZC, trainZF, trainTE, trainY, trainZCY, 
            valX, valZC, valZF, valTE, valY, valZCY, 
            testX, testZC, testZF, testTE, testY, testZCY, extdata) = utils.loadVolumeData2multi(args)
...
def model_define():
    ...
    ZCY = layers.Input(shape=trainZCY.shape[1:], dtype=tf.float32)
    TE = layers.Input(shape=trainTE.shape[1:], dtype=tf.int32) # int32

    Y = mymodels.ModelSetOurs(model_name=args.model_name, extdata=extdata, args=args, X=X, ZC=ZC, ZF=ZF, TE=TE, ZCY=ZCY)
    
    Y = Y * extdata['maxval']
    model = keras.models.Model((X, ZC, ZF, TE, ZCY), Y)
...
model.fit(
    (trainX, trainZC, trainZF, trainTE, trainZCY), trainY,
    batch_size=args.batch_size,
    epochs=args.max_epoch,
    validation_data=((valX, valZC, valZF, valTE, valZCY), valY),
...
pred = model.predict((testX, testZC, testZF, testTE, testZCY))
label = testY
