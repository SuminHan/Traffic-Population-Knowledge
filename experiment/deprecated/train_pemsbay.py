...
(trainX, trainZC, trainZF, trainTE, trainY, 
            ...
            testX, testZC, testZF, testTE, testY, extdata) = utils.loadVolumeDataB(args)
...
pred = np.tile(np.expand_dims(testX.mean(axis=1) * extdata['stdval'] + extdata['meanval'], 1), [1, args.Q, 1])
label = testY
print('Closeness Mean', utils.metric(pred, label), sep='\t')

pred = np.tile(testX[:, -1:, :]  * extdata['stdval'] + extdata['meanval'], [1, args.Q, 1])
label = testY
...
def model_define():
    ...
    Y = mymodels.ModelSetSTD(model_name=args.model_name, extdata=extdata, args=args, X=X, ZC=ZC, ZF=ZF, TE=TE)
